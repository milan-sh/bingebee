    ...
    return (
      ...
      <TweetForm channelId={channelId} fetchTweets={fetchTweets}/>
        ...
        <p className="text-gray-200 max-w-96 text-center">
          This channel has yet to make <span className="font-bold">Tweet</span>.
        </p>
      </TabsContent>
    );
  }
  ...
  return (
    <TabsContent value="tweets" className="flex flex-col gap-3">
      ...
      {tweets.map((tweet) => (
        <div
          key={tweet._id}
          className="border-b border-gray-700 py-3 flex gap-3"
        >
          <img
            src={tweet.owner?.avatar}
            alt={tweet.owner.fullName}
            className="h-14 min-w-14 rounded-full object-cover"
          />
          <div>
