...
const VideoUploadDialog = () => {
  ...
  const {register, handleSubmit, setValue, watch, setError, clearErrors, formState:{errors}, reset} = useForm<VideoUpload>({
    defaultValues:{
      title:"",
      description:"",
      videoFile:null,
  ...
  const onVideoDrop = useCallback(
    (acceptedFiles:File[], fileRejections:FileRejection[])=>{
      ...
      if(fileRejections.length>0){
        ...
        setError("videoFile", {type:'manual', message:rejection.errors[0]?.message || "Invalid file"})
      }
  ...
  return (
    <div className="absolute top-[40%] md:top-[50%]  left-[50%] -translate-x-[50%] -translate-y-[50%] h-[80vh] overflow-y-scroll bg-white border z-50 w-xs md:w-[60vw] max-w-3xl">
      <div className="rounded-none border bg-black text-white p-0">
        <form onSubmit={handleSubmit(onSubmit)} className="p-4">
        <div className="flex justify-between items-center p-2 border-b mb-2">
            <h2 className="text-xl font-semibold">Upload Video</h2>
            <Button type="submit" disabled={isLoading} className="disabled:bg-transparent">{isLoading ? <LoadingSvg/> : "Upload"}</Button>
        </div>
          {/* Vide Dropzone */}
            <div>
              ...
              <div
              {...getRootProps()}
              className={`border border-dashed p-10 text-center cursor-pointer ${isDragActive? 'border-blue-500' : "border"} ${errors.videoFile ? "border-red-500" : ""}`}
              >
                ...
                {isDragActive ? (
                  ...
                  <div>
                    ...
                    <p className="text-lg mb-3">
                      Drag & drop a video file here, or <span className="underline">click</span> to select 
                    </p>
                ...
                {errors.videoFile && <p className="text-red-600 my-0.5">{errors.videoFile.message as string}.</p>}
                {videoFilePreview && (
                  <div className="mt-4">
                    <video
            ...
            <textarea id="description" rows={5} className="border w-full mt-2 p-2"
