...
import shrek from "@/assets/shrek.gif"
  ...
  if (!videos || videos.length === 0) {
    return (
      <div className="text-white w-full mt-4 p-6 flex flex-col items-center gap-4">
        ...
        />
        <p className="md:text-lg font-serif text-accent/50 max-w-80 md:max-w-full text-center text-sm">
    ...
    <div className="w-full text-white p-6 mt-4 flex flex-col gap-3">
      {videos.map((video) => (
        <Link to={`/video/${video._id}`} key={video._id}>
          <div
            key={video._id}
            className="w-full grid grid-cols-1 md:grid-cols-7 md:gap-4 gap-2 border-b md:border-none border-gray-600"
          >
            <div className="relative md:col-span-3">
              <img
                src={video.thumbnail}
                alt={video.title}
                className="w-full min-h-56 max-h-56 object-cover rounded-sm"
              />
            ...
            <div className="md:col-span-4 p-2 md:p-0">
              ...
              <p className="text-sm text-gray-400 mt-5 md:block hidden">
                {video.description}
              </p>
            </div>
          </div>
